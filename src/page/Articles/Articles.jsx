...
const Articles = ({currentItems}) => {
  ...
  return (
    <div className='flex justify-center items-center w-full min-h-screen text-right articles-content'>
      <div className='flex flex-col p-2 2xl:w-[85%] sm:w-11/12 justify-center items-center'>
        ...
        <div className='flex sm:flex-wrap flex-col sm:flex-row justify-center gap-3 sm:gap-3 items-center'>
            {
                currentItems.map((article , index) => (
                    <div className='article w-[70%] sm:w-[60%] md:w-[30%] lg:w-[23%] 2xl:w-[15%] flex flex-col article mb-10' key={index} style={{boxShadow:'0px 0px 5px -2px #000'}}>
                        ...
                        <Link to={{pathname:'/articles/article', search:`?name=${article.title}`}} className='py-3 px-1 2xl:pt-5 text-gray-66 font-semibold  title hover:text-gray-88 line-clamp-1  transition-all'>{article.title}</Link>
                        <p className='text-gray-500 px-1 text-gray-88 text-sm line-clamp-3 xl-line-clamp-2  leading-loose  Description'>{article.explain}</p>
                        ...
                        <button className='flex gap-1 cursor-pointer flex-row w-full text-white  bg-green-bt py-2 px-2 sm:py-3 justify-center items-center hover:bg-sky-blue transition-all'>
                            <TfiMenuAlt className=''/>
                            <Link to={{pathname:'/articles/article', search:`name=${article.title}`}} className='text-[0.9rem] font-bold'>توضیحات بیشتر</Link>
                        </button>
                    </div>
                ))
            }
