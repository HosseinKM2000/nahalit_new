...
function AllArticles({ currentItems , articlesLength }) {
  ...
  return (
    <div className='w-full flex flex-col gap-5 items-center'>
        <span className='w-full text-white font-bold mx-5 md:m-0'> همه مقالات ( {articlesLength} )</span>
        <div className='w-full flex flex-col items-center gap-y-5 2xl:w-[70%]'>
          {
            currentItems.map((article,index) => (
              <div key={index} className='md:w-full gap-5 md:gap-0 w-[90%] sm:w-[60%] cursor-default text-[#1b243e] flex flex-col md:flex-row h-fit hover:bg-[#464e64] duration-300 hover:text-white transition-all shadow-[0px_5px_8px_1px_rgba(0,0,0,0.5)] items-center justify-between py-3 px-3 md:pl-3 md:pr-8   bg-[#f0f6f0] rounded-xl'>
                <img src={`${article?.image}`} alt={article?.title} className='w-full h-[17rem] md:w-[8rem] md:h-[8rem]'/>
